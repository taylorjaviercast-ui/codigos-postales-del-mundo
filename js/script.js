...
(function () {
  ...
  async function renderHome() {
    ...
    // Country image map - Unsplash small thumbnails (free, fast, legal)
    const countryImages = {
      'colombia': 'https://images.unsplash.com/photo-1568632234157-ce7aecd03d0d?w=400&q=70',
      'mexico': 'https://images.unsplash.com/photo-1518638150340-f706e86654de?w=400&q=70',
      'espana': 'https://images.unsplash.com/photo-1539037116277-4db20889f2d4?w=400&q=70',
      'usa': 'https://images.unsplash.com/photo-1485738422979-f5c462d49f04?w=400&q=70',
      'argentina': 'https://images.unsplash.com/photo-1589909202802-8f4aadce1849?w=400&q=70',
      'peru': 'https://images.unsplash.com/photo-1526392060635-9d6019884377?w=400&q=70',
      'brasil': 'https://images.unsplash.com/photo-1483729558449-99ef09a8c325?w=400&q=70',
      'chile': 'https://images.unsplash.com/photo-1510715220633-96e2e3ef2922?w=400&q=70',
      'ecuador': 'https://images.unsplash.com/photo-1566438480900-0609be27a4be?w=400&q=70',
      'venezuela': 'https://images.unsplash.com/photo-1571752726703-5e7d1f6a986d?w=400&q=70',
      'costa-rica': 'https://images.unsplash.com/photo-1519999482648-25049ddd37b1?w=400&q=70',
      'panama': 'https://images.unsplash.com/photo-1565214975484-3cfa9e56f914?w=400&q=70',
      'guatemala': 'https://images.unsplash.com/photo-1604530830209-3a4c9e66a9ef?w=400&q=70',
      'cuba': 'https://images.unsplash.com/photo-1500759285222-a95626b934cb?w=400&q=70',
      'republica-dominicana': 'https://images.unsplash.com/photo-1580237541049-2d715a09486e?w=400&q=70',
      'puerto-rico': 'https://images.unsplash.com/photo-1569839333583-7375336cde4b?w=400&q=70',
      'uruguay': 'https://images.unsplash.com/photo-1601579558498-c5af1907ef8d?w=400&q=70',
      'paraguay': 'https://images.unsplash.com/photo-1621451537084-482c73073a0f?w=400&q=70',
      'bolivia': 'https://images.unsplash.com/photo-1591378603223-e15b45a81640?w=400&q=70',
      'honduras': 'https://images.unsplash.com/photo-1608407390582-51e56e4e7d5c?w=400&q=70',
      'el-salvador': 'https://images.unsplash.com/photo-1603298414506-966e0e1aab7e?w=400&q=70',
      'nicaragua': 'https://images.unsplash.com/photo-1599062195011-620a5868eb8a?w=400&q=70'
    };

    grid.className = 'countries-image-grid';
    grid.innerHTML = data.countries.map((c, i) => {
      const img = countryImages[c.id] || '';
        ...
        <a href="country.html?id=${c.id}" class="country-image-card" style="animation-delay:${delay}s" aria-label="Ver códigos postales de ${c.name}">
          <div class="country-img-bg" ${img ? `style="background-image:url('${img}')"` : ''}>
            ...
            <div class="country-img-content">
              ...
              <h3 class="country-img-name">${c.name}</h3>
              <p class="country-img-cities">${c.cities.length} ciudades principales</p>
            </div>
            <div class="country-img-badge">${c.format}</div>
          </div>
          <div class="country-img-footer">
            <span>📮 ${c.format}</span>
  ...
  async function renderCountry() {
    ...
    // City list – Visual cards with images and staggered animation
    const cityList = document.getElementById('city-list');
    if (cityList) {
      // City gradient colors based on region
      const gradients = [
        'linear-gradient(135deg,#1e3a5f,#2563eb)','linear-gradient(135deg,#1e40af,#7c3aed)',
        'linear-gradient(135deg,#0f766e,#10b981)','linear-gradient(135deg,#9333ea,#ec4899)',
        'linear-gradient(135deg,#c2410c,#f59e0b)','linear-gradient(135deg,#0369a1,#06b6d4)',
        'linear-gradient(135deg,#4338ca,#6366f1)','linear-gradient(135deg,#15803d,#22c55e)',
        'linear-gradient(135deg,#b91c1c,#ef4444)','linear-gradient(135deg,#7e22ce,#a855f7)'
      ];

      cityList.className = 'city-cards-grid';
      cityList.innerHTML = country.cities.map((city, i) => {
        const grad = gradients[i % gradients.length];
        const delay = Math.min(i * 0.04, 1.5);
        return `
          <a href="city.html?country=${country.id}&city=${city.id}" class="city-visual-card" style="animation-delay:${delay}s">
            <div class="city-card-bg" style="background:${grad}">
              <div class="city-card-pattern"></div>
              <div class="city-card-content">
                <span class="city-card-flag">${country.flag}</span>
                <h4 class="city-card-name">${city.name}</h4>
                <span class="city-card-state">${city.state}</span>
              </div>
              <div class="city-card-code-badge">${city.postalCode}</div>
            </div>
            <div class="city-card-footer">
              <span class="city-card-range">📮 ${city.postalRange}</span>
              <span class="city-card-pop">${city.population ? '👥 ' + city.population : ''}</span>
            </div>
          </a>
        `;
      }).join('');
