...
(function () {
  ...
  function resolvePath(path) {
    // Always resolve from root - works on any hosting
    return '/' + path;
  }
  ...
  async function loadData() {
    if (DATA) return DATA;
    // Try multiple URL strategies to ensure data loads on any hosting
    const urls = [
      resolvePath('data/codigos.json') + '?v=' + DATA_VERSION,
      '/data/codigos.json?v=' + DATA_VERSION,
      'data/codigos.json?v=' + DATA_VERSION
    ];
    for (const url of urls) {
      try {
        console.log('[loadData] Trying:', url);
        const res = await fetch(url);
        if (!res.ok) { console.warn('[loadData] HTTP', res.status, 'for', url); continue; }
        DATA = await res.json();
        if (DATA && DATA.countries) {
          console.log('[loadData] OK via', url, '– countries:', DATA.countries.length);
          buildSearchIndex();
          return DATA;
        }
      } catch (e) {
        console.warn('[loadData] Failed:', url, e.message);
      }
    }
    console.error('[loadData] ALL URLs failed!');
    return null;
  }
  ...
  function initGames() {
    ...
    // Also bind on index.html (fact button)
    if (location.pathname.includes('index') || location.pathname === '/' || location.pathname.endsWith('/')) {
      var factBtnHome = document.getElementById('dato-curioso-btn');
      if (factBtnHome) factBtnHome.addEventListener('click', function(){ nextFact(); });
    }
  }
  ...
  function init() {
    ...
    if (path.includes('country')) {
      ...
      loadData().then(function(data) {
        console.log('[init] juegos.html – loadData result:', data ? 'OK' : 'FAILED');
        initGames();
      }).catch(function(e) {
        console.error('[init] juegos.html – loadData/initGames error:', e);
        // Even if loadData fails, try initGames so buttons show error state
        initGames();
      });
