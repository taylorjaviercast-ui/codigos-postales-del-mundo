...
(function () {
  ...
  let DATA = null;
  const CACHE = {};
  // Detect base path - works with both file.html and cleanUrls (no extension)
  const BASE_PATH = (() => {
    const p = location.pathname;
    // If path has a file extension, strip the filename
    if (/\.[^/]+$/.test(p)) return p.replace(/\/[^/]*$/, '');
    // If at root, return empty
    if (p === '/' || p === '') return '';
    // cleanUrls: /juegos, /city etc - treat as file, return parent
    return '';
  })();
